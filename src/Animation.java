...
public class Animation {
	...
	public Animation() { 
		this(new ArrayList<BufferedImage>(), 0); 
	}
	
	private Animation(ArrayList frames, float totalDuration2) 
	{
		this.frames = frames;
	...
	public static Animation ReadSheet(String filename, int tileHeight, int tileWidth, float duration)
	{
		...
		try { spritesheet = ImageIO.read( new File("Assets/" + filename + ".png") ); } 
		...
		for (int i = 0; i < sheetHeight; i++) 
		{
			...
			{
				anim.AddFrame( (BufferedImage) sheet.getSubimage(j * tileWidth, i * tileHeight, tileWidth, tileHeight), duration );
			}
		}
	...
	// Only copy a certain about of images
	...
	// Only copy a certain about of images, on a certain line.
	public static Animation ReadSheet(String filename, int tileHeight, int tileWidth, float duration, int row, int amt)
	{
		Animation anim =  new Animation();
		BufferedImage spritesheet = null;
		int iterator = 1;
		...
		for (int j = 0; j < sheetWidth; j++) 
		{
			anim.AddFrame( (BufferedImage) sheet.getSubimage(j * tileWidth, row * tileHeight, tileWidth, tileHeight), duration );
			iterator++;
			if (iterator == amt) {break;}
		}
	...
	public static BufferedImage ReadImage(String filename)
	{
		...
		try { sprite = ImageIO.read( new File("Assets/" + filename + ".png") ); } 
		catch (IOException e) {e.printStackTrace();}
    ...
    private BufferedImage GetScaledImage(BufferedImage image, float dx, float dy) 
    {
        ...
        AffineTransformOp scaleOperation = new AffineTransformOp(scaleInstance, AffineTransformOp.TYPE_NEAREST_NEIGHBOR);
    ...
    public int GetLength() 	    { return frames.size();  }
    public int GetCurrFrame()   { return currFrameIndex; }
    public boolean Ended() 	    { return GetLength() - 1 == GetCurrFrame(); }
